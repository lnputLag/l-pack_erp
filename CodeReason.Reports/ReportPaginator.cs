    ...
    public class ReportPaginator : DocumentPaginator
    {
        ...
        public ReportPaginator(ReportDocument report, ReportData data)
        {
            ...
            while (block != null)
            {
                ...
                if ((thisBlock == _blockPageHeader) || (thisBlock == _blockPageFooter) || hide) _flowDocument.Blocks.Remove(thisBlock);
            }
            ...
            /*
            ArrayList inlineObjects = _dynamicCache.GetFlowDocumentVisualListByInterface(typeof(IInlineObject));
            foreach (IInlineObject io in inlineObjects)
            {
                if (io == null) continue;
                var p=new Paragraph(new Run("DDDDDDD"));
                _flowDocument.Blocks.Add(p);
            } 
            */
                        ...
                        for (int j = 0; j < rowGroup.Rows.Count; j++)
                        {
                            ...
                            else
                            {
                                ...
                                foreach (TableCell cell in newTableRow.Cells)
                                {
                                    ...
                                    // DM 2020-08-18_F1
                                    // proc conditional table rows
                                    DocumentWalker localWalker2 = new DocumentWalker();
                                    List<ITableRowCondition> newBlocks = localWalker2.TraverseBlockCollection<ITableRowCondition>(cell.Blocks);
                                    foreach (TableRowConditional r in newBlocks)
                                    {
                                        if( !r.CheckConditionFulfilled(_data,table.TableName,i) )
                                        {
                                            r.Cells.Clear();
                                        }
                                    ...
                                    // DM 2020-08-18_F2
                                    // proc custom inlines
                                    DocumentWalker localWalker3 = new DocumentWalker();
                                    List<ISectionInline> inlineBlocks = localWalker3.TraverseBlockCollection<ISectionInline>(cell.Blocks);
                                    foreach (SectionInline ib in inlineBlocks)
                                    {
                                        ib.FillSection(_data,table.TableName,i);
                                    }  
                                ...
                                { 
                                    ...
                                    // proc cells styles
                                    DocumentWalker localWalker = new DocumentWalker();
                                    List<ITableCellStyled> items = localWalker.TraverseCellsCollection<ITableCellStyled>(newTableRow.Cells);
                                    foreach (TableCellStyled item in items)
                                    {
                                        item.AssignStyle(_data,table.TableName,row,i);
                                        //item.Style=(Style)row.FindResource("TableCellGreen");
                                        //var r1=item;
                                        //var rr=0;
                                    }  
