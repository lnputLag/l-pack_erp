...
namespace Client.Interfaces.Preproduction
{
    ...
    public partial class WebTechnologicalMapChat : UserControl
    {
        public WebTechnologicalMapChat()
        {
            InitializeComponent();
                ...
                if (result != null)
                {
                    ...
                    if (refDict != null)
                    {
                        if (refDict.Items.Count > 0)
                        {
                            ...
                            if (ChatEmailFlag)
                            {
                                SendToEmailCheckBox.IsEnabled = false;
                            }
                        }
                    }
                    else
        ...
        private async Task<bool> SendMessage()
        {
            ...
            {
                Message.Text = "";
                var d = new DialogWindow($"Заполните текст сообщения.", "Чат по веб-техкарте", "", DialogWindowButtons.OK);
                d.ShowDialog();
                resume = false;
            }
            if (resume && RecipientListTextBox.IsVisible == true && string.IsNullOrEmpty(RecipientListTextBox.Text))
            {
                var d = new DialogWindow($"Заполните получателей перед отправкой сообщения.", "Чат по веб-техкарте", "", DialogWindowButtons.OK);
                d.ShowDialog();
                resume = false;
            }
            if (resume)
        ...
        /// <summary>
        /// Обновляет флаг прочитанных сообщений для веб-техкарт
        /// </summary>
        private async void UpdateUnread()
        {
            ...
            if (q.Answer.Status == 0)
            {
                ...
                Central.Msg.SendMessage(new ItemMessage()
                {
                    ...
                    SenderName = "WebTechnologicalMapChat",
