...
{
    /// <summary>
    /// Вкладка с настройкой готовности штанцформ для производства
    /// </summary>
    /// <author>ryasnoy_pv</author>
    public partial class CuttingStampReadyTab : ControlBase
    {
        public CuttingStampReadyTab()
        {
            InitializeComponent();
        ...
        {
            var placeList = new Dictionary<string, string>()
            {
                { "0", "Все" },
                { "1", "2 этаж" },
                { "2", "Переработка" },
            };
            StoragePlaceSelectBox.Items = placeList;
        }
        ...
        private async void LoadItems()
        {
            ...
            if (q.Answer.Status == 0)
            {
                ...
                if (result != null)
                {
                    ...
                    {
                        int storageType = item.CheckGet("STORAGE_TYPE").ToInt();
                        string storagePlace = "Переработка";
                        if (storageType == 1)
                        {
                            storagePlace = "2 этаж";
                        }
                        item.CheckAdd("_STORAGE_PLACE", storagePlace);
                    }
                    Grid.UpdateItems(ds);
                }
            }
        ...
        private void FilterItems()
        {
            if (Grid.Items != null)
            {
                ...
                {
                    int storagePlaceId = StoragePlaceSelectBox.SelectedItem.Key.ToInt();

                    bool doFilteringByPlace = false;
                    if (storagePlaceId > 0)
                    {
                        doFilteringByPlace = true;
                    }

                    if (doFilteringByPlace)
                    {
                        var items = new List<Dictionary<string, string>>();
                        foreach (Dictionary<string, string> row in Grid.Items)
                        {
                            bool includeByPlace = true;
                            if (storagePlaceId > 0)
                            {
                                includeByPlace = false;
                                if (storagePlaceId == row.CheckGet("STORAGE_TYPE").ToInt())
                                {
                                    includeByPlace = true;
                                }
                            }
        ...
        private void StoragePlace_SelectedItemChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            Grid.UpdateItems();
        }
    }
