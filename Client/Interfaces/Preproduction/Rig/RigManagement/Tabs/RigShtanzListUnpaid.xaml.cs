...
{
    /// <summary>
    /// Список неоплаченных и неразрешенных к аказу штанцформ
    /// </summary>
    /// <author>ryasnoy_pv</author>
    public partial class RigShtanzListUnpaid : UserControl
    {
        public RigShtanzListUnpaid()
        {
            InitializeComponent();
        ...
        private void ProcessMessages(ItemMessage obj)
        {
            ...
            if (obj.ReceiverGroup.IndexOf("PreproductionShtanz") > -1)
            {
                if (obj.ReceiverName.IndexOf(TabName) > -1)
                {
                    switch (obj.Action)
        ...
        private async void LoadItems()
        {
            ...
            if (q.Answer.Status == 0)
            {
                ...
                if (result != null)
                {
                    var ds = ListDataSet.Create(result, "SHTANZ_UNPAID");
        ...
        public void UpdateActions(Dictionary<string, string> selectedItem)
        {
            ...
            {
                MarkPaymentButton.IsEnabled = SelectedItem.CheckGet("SHTANZ_PAYMENT_FLAG").ToInt() == 0;
                Grid.Menu["MarkPayment"].Enabled = SelectedItem.CheckGet("SHTANZ_PAYMENT_FLAG").ToInt() == 0;

                AllowOrderButton.IsEnabled = SelectedItem.CheckGet("SHTANZ_ORDER_RIG_FLAG").ToInt() == 0;
                Grid.Menu["AllowOrder"].Enabled = SelectedItem.CheckGet("SHTANZ_ORDER_RIG_FLAG").ToInt() == 0;
            }
        ...
        {
            var dw = new DialogWindow("Вы действительно хотите отметить оплату штанцформы?", "Оплата штанцформы", "", DialogWindowButtons.YesNo);
            if ((bool)dw.ShowDialog())
            {
                if (dw.ResultButton == DialogResultButton.Yes)
                {
                    ...
                    {
                        var de = new DialogWindow(q.Answer.Error.Message, "Разрешение на штанцформу");
                        de.ShowDialog();
                    }
                }
            }
        ...
        {
            var dw = new DialogWindow("Вы действительно хотите разрешить заказ штанцформы?", "Разрешение на штанцформу", "", DialogWindowButtons.YesNo);
            if ((bool)dw.ShowDialog())
            {
                if (dw.ResultButton == DialogResultButton.Yes)
                {
        ...
        private async void UpdateInvoiceFlag()
        {
            ...
            q.Request.SetParam("TECH_MAP_ID", SelectedItem.CheckGet("ID"));
            q.Request.SetParam("SHTANZ_INVOICE_FLAG", SelectedItem.CheckGet("SHTANZ_INVOICE_FLAG"));
        ...
        private void CopyToClipboardButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (Grid.GridItems.Count > 0)
            {
                foreach (var row in Grid.GridItems)
                {
                    ...
                    {
                        text = $"{text}\n{row.CheckGet("ARTICLE")} {row.CheckGet("PRODUCT_NAME")}";
                    }
                }
            }
