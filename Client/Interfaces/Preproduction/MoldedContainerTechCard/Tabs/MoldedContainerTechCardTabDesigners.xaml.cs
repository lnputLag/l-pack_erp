...
namespace Client.Interfaces.Preproduction
{
    ...
    public partial class TechnologicalMapTabDesigners : ControlBase
    {
        public TechnologicalMapTabDesigners()
        {
            InitializeComponent();
            ...
            TabName = "TechnologicalMapTabDesigners";
            OnLoad = () =>
            {
                LoadRef();
                LoadStatuses();
                    ...
                    Commander.SetCurrentGroup("standart");
                    {
                        Commander.Add(new CommandItem()
                        {
                            ...
                            Action = () =>
                            {
                                ...
                                if (id != 0)
                                {
                                    if (Grid.SelectedItem.CheckGet("STATUS_DESIGNER_ID").ToInt() != 1)
                                    {
                    ...
                    Commander.SetCurrentGroup("special");
                    {
                        ...
                        Commander.Add(new CommandItem()
                        {
                            ...
                            CheckEnabled = () =>
                            {
                                ...
                                var row = Grid.SelectedItem;
                                if (row.CheckGet("STATUS_DESIGNER_ID").ToInt() == 2)
                                {
                                    result = true;
                                }
                                return result;
                        ...
                        Commander.Add(new CommandItem()
                        {
                            ...
                            CheckEnabled = () =>
                            {
                                var result = false;
                                var row = Grid.SelectedItem;
                                if (row.CheckGet("STATUS").ToInt() == 1 || row.CheckGet("DESIGNER_EMPL_ID").ToInt() != Central.User.EmployeeId.ToInt())
                                {
                                    result = true;
                                }
                                return result;
                        ...
                        Commander.Add(new CommandItem()
                        {
                            ...
                            Description = "Открепить себя от техкарты",
                            AccessLevel = Role.AccessMode.FullAccess,
                            Action = () =>
                            {
                            ...
                            CheckEnabled = () =>
                            {
                                var result = false;
                                var row = Grid.SelectedItem;
                                if (row.CheckGet("DESIGN").ToInt() == 1 && row.CheckGet("DESIGNER_EMPL_ID").ToInt() == Central.User.EmployeeId.ToInt())
                                {
                                    result = true;
                                }
                                return result;
                        ...
                        Commander.Add(new CommandItem()
                        {
                            ...
                            Action = () =>
                            {
                                TechMapChangeDesignStatus(2);
                            },
                            CheckEnabled = () =>
                            {
                                var result = false;
                                var row = Grid.SelectedItem;
                                var design = row.CheckGet("HAS_DESIGN_FILE").ToBool();
                                var sticker = row.CheckGet("HAS_STICKER").ToBool();
                                var type = row.CheckGet("CONTAINER_TYPE_ID").ToInt();
                                if (row.CheckGet("DESIGN").ToInt() == 1 && (
                                    type==2 && design ||
                                    type == 3 && sticker ||
                                    type == 4 && design && sticker
                                    ) &&
                                    Central.User.EmployeeId.ToInt() == row.CheckGet("DESIGNER_EMPL_ID").ToInt()
                                )
                                {
                                    result = true;
                                }
                        ...
                        Commander.Add(new CommandItem()
                        {
                            Name = "show_design",
                            Enabled = false,
                            Title = "Открыть файл дизайна печати",
                            MenuTitle = "Открыть файл дизайна печати",
                            Group = "fileoperation",
                            MenuUse = true,
                            ButtonName = "",
                            ButtonUse = false,
                        ...
                        Commander.Add(new CommandItem()
                        {
                            Name = "bind_design",
                            Enabled = false,
                            Title = "Привязать файл дизайна печати",
                            Group = "fileoperation",
                            MenuUse = true,
                            ButtonName = "",
                            ButtonUse = false,
        ...
        private void GridInit()
        {
            var columns = new List<DataGridHelperColumn>
            {
                ...
                new DataGridHelperColumn
                {
                    Header="Статус для дизайнеров",
                    Path="STATUS_DESIGNER_ID",
                    ColumnType=ColumnTypeRef.Integer,
                    Hidden=true,
                },
            };
            ...
            Grid.RowStylers = new Dictionary<DataGridHelperColumn.StylerTypeRef, DataGridHelperColumn.StylerDelegate>()
            {
                // Цвета фона строк
                {
                    ...
                    row =>
                    {
                        ...
                        var color = "";
                        int currentStatus = 0;
                        currentStatus = row.CheckGet("STATUS_DESIGNER_ID").ToInt();
        ...
        public async void LoadRef()
        {
            ...
            if (q.Answer.Status == 0)
            {
                ...
                if (result != null)
                {
                    ...
                    var designers = ListDataSet.Create(result, "DESIGNERS");
                    var designersList = new Dictionary<string, string>()
                    {
                        { "-1", "Все" },
                    };
                    foreach (var item in designers.Items)
                    {
                        designersList.CheckAdd(item["ID"].ToInt().ToString(), item.CheckGet("FIO"));
                    }
                    DesignerName.Items = designersList;
                    DesignerName.SetSelectedItemByKey("-1");
                }
            }
        }
        ...
        public async void LoadItems()
        {
            ...
            q.Request.SetParam("Module", "Preproduction");
            q.Request.SetParam("Object", "MoldedContainer");
            q.Request.SetParam("Action", "ListTechCardForDesigner");

            q.Request.SetParam("SHOW_ARCHIVED", showArchived);
            q.Request.SetParam("STATUS", TkStatus.SelectedItem.Key.ToString());
            q.Request.SetParam("EMPL_ID", DesignerName.SelectedItem.Key?.ToString());
        ...
        private async void BindDesign(int id)
        {
            ...
            fd.Filter = "Файлы дизайна (*.*)|*.cdr;*.ai|Все файлы (*.*)|*.*";
            fd.FilterIndex = 0;
