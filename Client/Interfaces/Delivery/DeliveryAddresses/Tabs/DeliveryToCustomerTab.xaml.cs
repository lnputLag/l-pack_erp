...
namespace Client.Interfaces.DeliveryAddresses
{
    ...
    public partial class DeliveryToCustomerTab : ControlBase
    {
        public DeliveryToCustomerTab()
        {
            InitializeComponent();
            ...
            ControlSection = "delivery_to_customer";
            RoleName = "[erp]delivery_addresses";
            ControlTitle = "Доставка покупателю";
            ...
            OnUnload = () =>
            {
                ConsigneeGrid.Destruct();
                ShippingAddressGrid.Destruct();
            };
            //OnFocusGot = () =>
            //{
            //    ConsigneeGrid.ItemsAutoUpdate = true;
            //    ConsigneeGrid.Run();
            //};
            //OnFocusLost = () =>
            //{
            //    ConsigneeGrid.ItemsAutoUpdate = false;
            //};
            OnNavigate = () =>
            {
                var consigneeId = Parameters.CheckGet("consignee_id");
                if (!consigneeId.IsNullOrEmpty())
                {
                    ConsigneeSearchText.Text = consigneeId;
                    ConsigneeGrid.UpdateItems();
                }
            ...
            Commander.SetCurrentGridName("ConsigneeGrid");
            Commander.Add(new CommandItem()
            {
                Name = "consignee_refresh",
                Group = "grid_base",
                Enabled = true,
                Title = "Обновить",
                Description = "Обновить данные грузополучателей",
                ButtonUse = true,
                ButtonName = "ConsigneeRefreshButton",
                MenuUse = true,
                Action = () =>
                {
                    ConsigneeGrid.LoadItems();
                },
            });
            ...
            Commander.Add(new CommandItem()
            {
                ...
                Action = () =>
                {
                    ...
                    }, ConsigneeGrid.SelectedItem.CheckGet("ID_POK").ToInt(), false);
                },
                CheckEnabled = () =>
                {
                    var result = false;
                    var k = ConsigneeGrid.GetPrimaryKey();
                    var row = ConsigneeGrid.SelectedItem;
                ...
                Action = () =>
                {
                    ...
                    if (id != 0)
                    {
                        new ShippingAddressForm(new ItemMessage()
                        {
                            ReceiverName = ControlName,
                            Action = "shipping_address_refresh",
                        }, ConsigneeGrid.SelectedItem.CheckGet("ID_POK").ToInt(), false, id);
                    }
                },
                CheckEnabled = () =>
                {
        ...
        public void ConsigneeGridInit()
        {
            ...
            ConsigneeGrid.SetColumns(columns);
            ConsigneeGrid.SetPrimaryKey("ID_POK");
            //ConsigneeGrid.SetSorting("ID_POK", System.ComponentModel.ListSortDirection.Ascending);
            ConsigneeGrid.SearchText = ConsigneeSearchText;
            ConsigneeGrid.Toolbar = ConsigneeGridToolbar;
            ConsigneeGrid.Commands = Commander;
            ConsigneeGrid.ColumnWidthMode = GridBox.ColumnWidthModeRef.Full;
            ConsigneeGrid.AutoUpdateInterval = 0;
            ConsigneeGrid.ItemsAutoUpdate = false;
            ConsigneeGrid.QueryLoadItems = new RequestData()
            {
                Module = "Delivery",
                Object = "Consignee",
                Action = "List",
            ...
            ConsigneeGrid.OnSelectItem = (row) =>
            {
                ShippingAddressGrid.LoadItems();
            };
            ConsigneeGrid.Init();
                ...
                BeforeRequest = (RequestData rd) =>
                    {
                        ...
                        rd.Params = new Dictionary<string, string>()
                            {
                                { "ID_POK", ConsigneeGrid.SelectedItem.CheckGet("ID_POK") },
                            };
                    },
        ...
        private async void DeleteShippingAddress(Dictionary<string, string> row = null)
        {
        ...
        private void ShippingAddressPasteButtonClick(object sender, RoutedEventArgs e)
        {
            if (IdToCopy.Count > 0)
            {
                ...
                q.Request.SetParam("ID_POK", ConsigneeGrid.SelectedItem["ID_POK"]);
