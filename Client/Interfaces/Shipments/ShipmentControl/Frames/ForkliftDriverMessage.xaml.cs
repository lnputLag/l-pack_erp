...
namespace Client.Interfaces.Shipments
{
    ...
    /// (выбор водителя для отправки сообщения)
    /// </summary>
    /// <author>balchugov_dv</author>
    /// <version>1</version>
    /// <released>2022-11-18</released>
    /// <changed>2022-11-18</changed>
    public partial class ForkliftDriverMessage : UserControl
    {
        public ForkliftDriverMessage()
        {
            WindowTitle="Водители погрузчиков";
        ...
        public void Destroy()
        {
            ...
            Messenger.Default.Send(new ItemMessage
            {
                ...
                ReceiverName = "",
                SenderName = "ForkliftDriverMessage",
                Action = "Closed",
            });
        ...
        private void ProcessMessages(ItemMessage m)
        {
            ...
            {
                //object
                if(m.ReceiverName.IndexOf("ForkliftDriverMessage") > -1)
                {
                    switch(m.Action)
                    {
                        case "Refresh":
        ...
        {
            Central.ShowHelp("/doc/l-pack-erp/shipments/control/common/otpravit-soobschenie-voditelju-pogruzchika");
        }
        ...
        public void Save()
        {
...

            {
                ...
                if(list.Count > 0)
                {
                    ...
                    {
                        var id=row.CheckGet("MACHINE_ID").ToInt();
                        if(id != 0)
                        {
                            ids.Add(id.ToString());
                        }
                ...
                else
                {
                    ...
                    {
                        var id=Grid.SelectedItem.CheckGet("MACHINE_ID").ToInt();
                        if(id != 0)
                        {
                            ids.Add(id.ToString());
                        }
            ...
            if(ids.Count > 0)
            {
                var form = new ForkliftdriverSendMessage();
                form.IdDriverList.AddRange(ids);
                form.Show();
            }
