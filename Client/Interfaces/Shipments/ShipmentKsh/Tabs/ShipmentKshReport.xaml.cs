...
{
    /// <summary>
    /// Управление отгрузками, вкладка "Отчет" площадки Кашира
    /// </summary>
    /// <author>sviridov_ae</author>
    public partial class ShipmentKshReport : UserControl
    {
        public ShipmentKshReport()
        {
            InitializeComponent();
        ...
        public void Destroy()
        {
            ...
            {
                ReceiverGroup= "shipment_control_ksh",
                ReceiverName = "",
                SenderName = "ShipmentKshReport",
                Action = "Closed",
            });
        ...
        public void ProcessMessages(ItemMessage message)
        {
            if(message!=null)
            {
                if(
                    ...
                    && message.ReceiverName == "ShipmentKshReport"
                )
                {
        ...
        private void _ProcessMessages(ItemMessage m)
        {
            ...
            {
                if (m.ReceiverName.IndexOf("ShipmentKshReport") > -1)
                {
                    switch (m.Action)
                    {
                        case "Refresh":
        ...
        public async void LoadItems()
        {
            ...
            if (resume)
            {
                ...
                await Task.Run(() =>
                {
                    q = _LPackClientDataProvider.DoQueryGetResult("Shipments/ShipmentKsh", "Shipment", "ListReport", "Items", p);
                });
