...
{
    /// <summary>
    /// Графики скорости работы БДМ
    /// </summary>
    /// <author>Greshnyh_ni</author>   
    public partial class PaperMakingSpeedChart : UserControl
    {
        public PaperMakingSpeedChart()
        {
            InitializeComponent();
        }
...
*/
                ...
                SpeedGraph.XAxis.Step = 120; //120;

                if (MachineId.ToInt() == 1716)
                    SpeedGraph.YAxis.Step = 200;
                else
                    SpeedGraph.YAxis.Step = 100;
        ...
        public async void LoadData()
        {
            ...
            /*
                        ...
                        q.Request.SetParam("Action", "MachineListSpeed");
            ...
            try
            {
                var q = await LPackClientQuery.DoQueryAsync("ProductionPm", "Monitoring", "MachineListSpeed", "SPEED", p);
                        ...
                        if (ds.Items.Count > 0)
                        {
                            ...
                            if (!dateCurrent.IsNullOrEmpty())
                            {
                                ...
                                foreach (var row in ds.Items)
                                {
                                    ...
                                    // Central.Dbg($"    {row.CheckGet("DTTM")} {row.CheckGet("_SECONDS")} {row.CheckGet("SPEED_CURRENT")} ");
                                }
                            }

                            SpeedGraph.YAxis.Max = maxSpeed + 150;
