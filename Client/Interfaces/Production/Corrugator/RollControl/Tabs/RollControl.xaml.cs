...
namespace Client.Interfaces.Production
{
    ...
    public partial class RollControl:UserControl
    {
        ...
        {
            TabUpdate();
            Central.Dbg("SetActive RollControl");
            Central.WM.SelectedTab = "RollControl";
            ReturnTimerRun();
        }

        public string RoleName = "[erp]reel_control";
            ...
            if(LoadItemsTimerInterval != 0)
            {
                if(LoadItemsTimer == null)
                {
...

                    {
                        ...
                        row.CheckAdd("DESCRIPTION", "");
                        Central.Stat.TimerAdd("RollControl_LoadItemsRun", row);
                    }
                ...
                if(ReturnTimerInterval != 0)
                {
                    if(ReturnTimer == null)
                    {
...

                        {
                            ...
                            row.CheckAdd("DESCRIPTION", "");
                            Central.Stat.TimerAdd("RollControl_ReturnTimerRun", row);
                        }
            ...
            if(UnblockTimerInterval != 0)
            {
                if(UnblockTimer == null)
                {
...

                    {
                        ...
                        row.CheckAdd("DESCRIPTION", "");
                        Central.Stat.TimerAdd("RollControl_UnblockTimerRun", row);
                    }
        ...
        private void MoveRoll()
        {
            ...
            )
            {
                ...
                if (CurrentMachineId == 22)
                {
                    string msg = $"\nВНИМАНИЕ!\n\nРулон отправляется на склад АВТОМАТИЧЕСКИ!\nИспользуйте отправку рулона на склад вручную только в нештатных ситуациях!";
                    var d = new DialogTouch($"{msg}", "", "", DialogWindowButtons.OK);
                    d.Show();
                    d.OnComplete = (DialogResultButton resultButton) =>
                    {
                        i.Edit();
