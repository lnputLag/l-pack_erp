...
namespace Client.Interfaces.Production.Corrugator.TaskPlanningKashira
{
    ...
    public class TaskPlaningDataSet
    {
        ...
        /// </summary>
        public enum TypeStanok { Unknow = 0, Js=23 };
        ...
        //CHECK THIS PLACE (Хотят слухи, что это вообще не используется в авто планировании)
        // Хотя не я использую для автопланированию ключ
        ...
        public bool Undo()
        {
            ...
            if (res)
            {
                ...
                UpdateGrid?.Invoke(TypeStanok.Js);
            }
                            ...
                            foreach (var item in Items)
                            {
                                ...
                                if (curItem != null)
                                {
                                    ...
                                    if (curItem[Dictionary.Format].ToInt() != AfterRowIndex)
                                    {
                                        ...
                                        var changeFormat = LPackClientQuery.DoQueryAsync("Production", "TaskPlanningKashira", "UpdateTaskFormat", "ITEMS",
                            ...
                            if (resume)
                            {
                                foreach (var item in Items)
                                {
                                    ...
                                    if (curItem != null)
                                    {
                                        ...
                                        var deleteTask = LPackClientQuery.DoQueryAsync("Production", "TaskPlanningKashira", "DeleteTask", string.Empty,
                                            new Dictionary<string, string>()
                                            {
                        ...
                        if(AfterRowIndex > 0)
                        {
                            ...
                            if (curItem != null)
                            {
                                ...
                                if (source != destination)
                                {
                                    ...
                                    // Я пока точно не могу ответить за что конкретно отвечает данная функция, но я постараюсь
                                    if(destination== (int)TypeStanok.Js)
                                    {
        ...
        public async Task<List<Dictionary<string, string>>> GetAllOrders()
        {
            ...
            var list1 = await GetDataAsync(TypeStanok.Js);
        ...
        private async Task<ConcurrentBag<Dictionary<string, string>>> LoadDataAsync(CancellationToken token, ConcurrentBag<Dictionary<string, string>> dataContainer, bool callEvents = true )
        {
            ...
            var qd = await LPackClientQuery.DoQueryAsync("Production", "TaskPlanningKashira", "ListDeletedTask", "ITEMS");
            ...
            tasks.Add(LPackClientQuery.DoQueryAsync("Production", "TaskPlanningKashira", "List", "ITEMS", new Dictionary<string, string>() { { Dictionary.StanokId, "-1" }, { "START_DATE", StartCalculatedDateTime.ToString("dd.MM.yyyy HH:mm:ss") }, { "END_DATE", EndCalculatedDateTime.ToString("dd.MM.yyyy HH:mm:ss") } }, Central.Parameters.RequestGridTimeout * 100));
        ...
        private async void PostProcessData(TypeStanok typeStanok, bool callEvents)
        {
            if (CurrentData != null)
            {
                ...
                .ForEach(x =>
                {
                    listOfTask.Add(LPackClientQuery.DoQueryAsync("Production", "TaskPlanningKashira", "ListRawMaterial", "ITEMS", new Dictionary<string, string>() { { "ID_PZ", x.CheckGet(Dictionary.ProductionTaskId).ToString() } }));
                });
        ...
        internal void SetLastDataTime(TypeStanok currentMachineId, DateTime lastDateTime, double lastDurationMinutes)
        {
            ...
            if(currentMachineId!= TypeStanok.Unknow && CurrentData != null)
            {
                ...
                if(freeOrders.Any())
                {
                    freeOrders.ForEach(x =>
                    {
                        ...
                        if (currentMachineId == TypeStanok.Js)
                        {
                            if (otherMachine == 8)
                            {
