...
{
    /// <summary>
    /// Форма списка комплектаций
    /// Операция печати чека (в процессе)
    /// Операция осуществления комплектации (в процессе)
    /// </summary>
    /// <author>михеев</author>
    public partial class ComplectationList : UserControl
    {
        public ComplectationList()
        {
            InitializeComponent();
        ...
        public void SetDefaults()
        {
...

            {
                ...
                list.Add(ComplectationPlace.MoldedContainer, "ЛТ");
        ...
        private void ComplectationGridInit()
        {
            var columns = new List<DataGridHelperColumn>
            {
                ...
                {
                    Header = "Артикул",
                    Path = "consumption_artikul".ToUpper(),
                    ColumnType = DataGridHelperColumn.ColumnTypeRef.String,
                    MinWidth = 120,
                    MaxWidth = 120,
                },
        ...
        public async void RemoveComplectation()
        {
            ...
            if (q.Answer.Status == 0)
            {
                ...
                if (result != null)
                {
                    ...
                    // СГП
                    if (ds.Items.First().CheckGet("ID_ST").ToInt() == 720)
                    {
                    ...
                    // Переработка
                    else if (ds.Items.First().CheckGet("ID_ST").ToInt() == 721)
                    {
                        // отправить сообщение списку доступных комплектаций на ГА
                        Messenger.Default.Send(new ItemMessage
                        {
                    ...
                    // ГА
                    else if (ds.Items.First().CheckGet("ID_ST").ToInt() == 719)
                    {
                        // отправить сообщение списку доступных комплектаций на ГА
                        Messenger.Default.Send(new ItemMessage
                        {
        ...
        public void PrintLabel(int mode=1)
        {
