...
namespace Client.Interfaces.Production
{
    ...
    /// Комплектация ГА
    /// </summary>
    public partial class ComplectationCorrugator : UserControl
    {
        public ComplectationCorrugator()
        {
            InitializeComponent();
            RoleName = "[erp]complectation_cm";
            FrameName = "ProductionComplectationCM";
            ...
            ComplectationCorrugatorInStockFlagTextBlock.Text = "Поддоны в ячейке К -2";
            ComplectationCorrugatorInStockFlagTextBlock.Visibility = Visibility.Hidden;
        ...
        /// <summary>
        /// Таймер для предупреждения о поддонах в К-2
        /// </summary>
        public DispatcherTimer ComplectationWarningTimer { get; set; }

        /// <summary>
        /// интервал обновления таймера для предупреждения о поддонах в К-2 (сек)
        /// </summary>
        ...
        private void ProcessMessages(ItemMessage m)
        {
            if (m.ReceiverGroup.Contains("Complectation"))
            {
                ...
                else if (m.ReceiverName.Contains("CM"))
                {
                    switch (m.Action)
                    {
                        case "Refresh":
            ...
            if (ComplectationWarningTimerInterval != 0)
            {
                if (ComplectationWarningTimer == null)
                {
...

                    {
                        ...
                        Central.Stat.TimerAdd("ComplectationCm_RunComplectationWarningTimer", row);
                    }
        ...
        public async void GetComplectationWarningData()
        {
            ...
            q.Request.SetParam("Action", "CountComplectationCorrugatorInStock");
            ...
            if (q.Answer.Status == 0)
            {
                ...
                {
                    // Работаем со отображением значка К-2
                    var complectationWarningDs = ListDataSet.Create(result, "ITEMS");
                    if (complectationWarningDs != null && complectationWarningDs.Items != null && complectationWarningDs.Items.Count > 0)
                    {
                        if (complectationWarningDs.Items[0].CheckGet("COUNT").ToInt() > 0)
                        {
                            ComplectationCorrugatorInStockFlagTextBlock.Visibility = Visibility.Visible;
                        }
                        else
                        {
                            ComplectationCorrugatorInStockFlagTextBlock.Visibility = Visibility.Hidden;
        ...
        private void Close()
        {
            Central.WM.RemoveTab($"ComplectationCMView_new");
            Destroy();
        }
        ...
        private void Destroy()
        {
            ...
            Messenger.Default.Send(new ItemMessage
            {
                ...
                ReceiverName = "",
                SenderName = "ComplectationCMEdit",
                Action = "Closed",
            });
        ...
        private void Show()
        {
            var title = "Новая комплектация на ГА";
            Central.WM.AddTab($"ComplectationCMView_new", title, true, "add", this);
        }
