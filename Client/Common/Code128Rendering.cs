...
namespace Client.Common
{
	...
	public static class Code128Rendering
	{
      ...
      public static System.Drawing.Bitmap MakeBarcodeImage( string InputData, int BarWeight, bool AddQuietZone )
      {
         ...
         System.Drawing.Bitmap myimg = new System.Drawing.Bitmap(width, height);
        ...
        /// <param name="AddQuietZone">Add required horiz margins (use if output is tight)</param>
        /// <returns>An Image of the Code128 barcode representing the message</returns>
        public static System.Drawing.Bitmap MakeBarcodeImage(string InputData, float BarWeight, bool AddQuietZone)
        {
            // get the Code128 codes to represent the message
            Code128Content content = new Code128Content(InputData);
            int[] codes = content.Codes;
            ...
            float width, height;
            width = (((float)codes.Length - 3) * 11 + 35) * BarWeight;
            height = Convert.ToInt32(System.Math.Ceiling(Convert.ToSingle(width) * .15F));

            if (AddQuietZone)
            {
                width += 2 * (float)cQuietWidth * BarWeight;  // on both sides
            }

            // get surface to draw on
            System.Drawing.Bitmap myimg = new System.Drawing.Bitmap(width.ToInt(), height.ToInt()); //Возможно стоит округлять вверх до целого числа
            using (Graphics gr = Graphics.FromImage(myimg))
            {
                ...
                // skip quiet zone
                float cursor = AddQuietZone ? (float)cQuietWidth * BarWeight : 0;
                ...
                for (int codeidx = 0; codeidx < codes.Length; codeidx++)
                {
                    ...
                    for (int bar = 0; bar < 8; bar += 2)
                    {
                        float barwidth = (float)cPatterns[code, bar] * BarWeight;
                        float spcwidth = (float)cPatterns[code, bar + 1] * BarWeight;
